# Archived Prototype: PracticeArena

This directory is a source-only archive of the pre-FastAPI Flask MVP that previously lived at `backend/PracticeArena`.

- Archived on `2026-04-11` during the backend restructure.
- Runtime artifacts and nested-repo files were intentionally excluded from this copy: `.git/`, `.venv/`, `.env`, `*.sqlite3`, `__pycache__/`, `.DS_Store`.
- This code is kept for reference only and is not part of the active backend startup path.
