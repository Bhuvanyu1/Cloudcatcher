...
class GCPConnector(CloudConnector):
    def __init__(self, project_id: str, service_account_json: Any, account_id: str):
        self.project_id = project_id
        self.account_id = account_id
        self.client = self._build_client(service_account_json)

    @staticmethod
    def _build_client(service_account_json: Any) -> compute_v1.InstancesClient:
        ...
        if isinstance(service_account_json, str):
            ...
            if payload.startswith("{"):
                ...
                return compute_v1.InstancesClient(credentials=credentials)
            return compute_v1.InstancesClient.from_service_account_json(payload)

        raise ValueError("service_account_json must be a dict, JSON string, or file path")
...
def build_connector(provider: str, credentials: Dict[str, Any], account_id: str) -> CloudConnector:
    ...
    if normalized_provider in {"digitalocean", "do"}:
        return DigitalOceanConnector(token=credentials["token"], account_id=account_id)

    raise ValueError(f"Unsupported provider: {provider}")


async def fetch_instances(provider: str, credentials: Dict[str, Any], account_id: str) -> List[Dict[str, Any]]:
    connector = build_connector(provider, credentials, account_id)
    return await connector.list_instances()
