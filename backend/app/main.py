...
from .services.wafr import WAFREngine
security = HTTPBearer()
...
async def update_cloud_account(account_id: str, update_data: CloudAccountUpdate):
    ...
    if "credentials" in update_dict:
        update_dict["credentials"] = encrypt_credentials(update_dict["credentials"])
...
async def sync_all_accounts():
    ...
    for account in accounts:
        try:
            ...
            raw_credentials = account.get("credentials", {})
            credentials = (
                decrypt_credentials(raw_credentials)
...
async def run_wafr_assessment(account_id: str):
    ...
    raw_credentials = account["credentials"]
    credentials = (
        decrypt_credentials(raw_credentials)
        if isinstance(raw_credentials, str)
        else raw_credentials
